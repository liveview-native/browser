...
_install-netsurf: clean-netsurf
	...
	BUILDDIR=$(BC_NS)/build/libwapcaplet make CC=$(CC) AR=$(AR) RANLIB=$(RANLIB) install && \
	cd ../libparserutils && \
	printf "\e[33mInstalling libparserutils...\e[0m\n" && \
	BUILDDIR=$(BC_NS)/build/libparserutils make CC=$(CC) AR=$(AR) RANLIB=$(RANLIB) install && \
	cd ../libhubbub && \
	printf "\e[33mInstalling libhubbub...\e[0m\n" && \
	BUILDDIR=$(BC_NS)/build/libhubbub make CC=$(CC) AR=$(AR) RANLIB=$(RANLIB) install && \
	rm src/treebuilder/autogenerated-element-type.c && \
	cd ../libdom && \
	printf "\e[33mInstalling libdom...\e[0m\n" && \
	BUILDDIR=$(BC_NS)/build/libdom make CC=$(CC) AR=$(AR) RANLIB=$(RANLIB) install && \
...
install-libiconv: download-libiconv clean-libiconv
	...
	./configure --prefix=$(ICONV) --enable-static --disable-shared --host=arm-apple-darwin && \
	make && make install
...
_build_mimalloc: clean-mimalloc
	...
	cmake \
		...
		-DMI_BUILD_SHARED=OFF -DMI_BUILD_OBJECT=OFF -DMI_BUILD_TESTS=OFF -DMI_OVERRIDE=OFF $(OPTS) ../../.. && \
	make && \
